      ...
      <div className="mt-6 space-y-5">
        ...
        <section className="space-y-4 rounded-2xl border border-white/10 bg-black/10 p-4">
          ...
          {activeBackground ? <div className="relative overflow-hidden rounded-2xl border border-amber-200/20 bg-black/20 p-3">
            <div className="relative aspect-[16/9] overflow-hidden rounded-xl" style={previewStyle}><button type="button" onClick={() => moveBackground(-1)} aria-label="پس‌زمینه قبلی" className="absolute right-2 top-1/2 -translate-y-1/2 rounded-full bg-black/55 p-2 backdrop-blur hover:bg-black/75"><ChevronRight className="h-5 w-5" /></button><button type="button" onClick={() => moveBackground(1)} aria-label="پس‌زمینه بعدی" className="absolute left-2 top-1/2 -translate-y-1/2 rounded-full bg-black/55 p-2 backdrop-blur hover:bg-black/75"><ChevronLeft className="h-5 w-5" /></button></div>
