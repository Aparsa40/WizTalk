...
export function CharacterSelector({ characters, onSelect, onManage, onLogout }: CharacterSelectorProps) {
  return <section dir="rtl" className="relative flex min-h-[100dvh] flex-col overflow-x-hidden bg-[#12091f] px-4 py-6 text-amber-50 sm:px-8 sm:py-10 lg:py-12">
    ...
    <div className="relative z-10 mx-auto mb-7 flex w-full max-w-6xl flex-col gap-5 sm:mb-10 sm:gap-6 md:flex-row md:items-center md:justify-between">
      ...
      <div className="flex w-full gap-2 md:w-auto"><button type="button" onClick={onManage} className="flex flex-1 items-center justify-center gap-2 rounded-xl border border-amber-200/20 bg-white/5 px-4 py-3 text-sm text-amber-100 transition hover:border-amber-300/60 hover:bg-amber-400/10 md:flex-none"><Settings2 className="h-4 w-4"/>مدیریت</button><button type="button" onClick={onLogout} className="flex items-center justify-center gap-2 rounded-xl border border-white/10 px-4 py-3 text-sm text-amber-50/65 hover:bg-white/5"><LogOut className="h-4 w-4"/>خروج</button></div>
    </div>
    <div className="relative z-10 mx-auto grid w-full max-w-6xl grid-cols-1 gap-4 sm:gap-6 md:grid-cols-2 lg:grid-cols-3">
      {characters.map((character) => <button type="button" key={character.identity.id} onClick={()=>onSelect(character)} className="group min-w-0 overflow-hidden rounded-3xl border border-white/10 bg-white/[0.06] text-right shadow-xl backdrop-blur-md transition duration-300 hover:-translate-y-1 hover:border-amber-300/60 hover:bg-white/[0.1] hover:shadow-amber-900/30">
        <div className="relative h-72 overflow-hidden bg-[#241437] sm:h-80"><img src={character.avatar.thumbnail ?? character.avatar.source} alt={character.identity.displayName} className="h-full w-full object-contain transition duration-500 group-hover:scale-[1.02]" onError={(event)=>{if(character.avatar.fallbackSource)event.currentTarget.src=character.avatar.fallbackSource;}}/><div className="absolute inset-x-0 bottom-0 h-28 bg-gradient-to-t from-[#1b0e2b] to-transparent"/><span className="absolute bottom-3 right-3 max-w-[calc(100%-1.5rem)] truncate rounded-full bg-black/45 px-3 py-1 text-xs text-amber-100 backdrop-blur-sm sm:bottom-4 sm:right-4">{character.identity.role}</span></div>
        <div className="p-4 sm:p-5"><h2 className="truncate text-lg font-bold text-amber-200 sm:text-xl">{character.identity.displayName}</h2><p className="mt-2 min-h-12 text-sm leading-6 text-amber-50/70 sm:mt-3">{character.identity.description}</p><span className="mt-4 inline-flex text-sm font-medium text-amber-300 sm:mt-5">شروع Session جدید ←</span></div>
      </button>)}
    </div>
    {!characters.length && <div className="relative z-10 mx-auto rounded-2xl border border-amber-200/20 bg-white/5 p-6 text-center text-sm text-amber-100/70">شخصیتی برای نمایش وجود ندارد.</div>}
