...
export function ChatUI({ character, onBack, onOpenSettings }: ChatUIProps) {
  ...
  useEffect(() => {
    ...
    return () => {
      voiceManager.abortListening(); voiceManager.stop();
  ...
  const handleSend = async () => {
    ...
    setMessages(next); MemoryService.saveMessage(character.identity.id, user); setInput(''); setIsTyping(true); controller.current.setState('thinking');
    try {
      ...
      setMessages((current) => [...current, reply]); MemoryService.saveMessage(character.identity.id, reply); await speak(reply.text);
    } catch (error) {
      console.error('Chat request failed', error);
      // Response fallback belongs to the server; never expose raw provider errors in UI.
      controller.current.setState('idle');
    } finally { setIsTyping(false); }
  };

  const toggleListening = () => {
    if (isListening) { voiceManager.stopListening(); setIsListening(false); controller.current.setState('idle'); return; }
    const recognition = voiceManager.initSpeechToText(character,
      (text) => setInput((value) => (value ? `${value} ${text}` : text)),
      (message) => { console.warn('Speech recognition error:', message); setIsListening(false); controller.current.setState('idle'); },
      () => { setIsListening(false); if (!isTyping) controller.current.setState('idle'); },
    );
    if (!recognition || !voiceManager.startListening()) { setIsListening(false); controller.current.setState('idle'); return; }
    setIsListening(true); controller.current.setState('listening');
  };
      ...
      <section className="relative flex min-h-0 flex-1 flex-col items-center justify-center overflow-hidden px-3 pb-28 pt-5 sm:px-6 sm:pb-32 sm:pt-7">
        ...
        <div className="relative z-10 flex w-full max-w-2xl flex-col items-center">
          ...
          <div className="relative -mt-2 w-[min(90vw,38rem)] sm:-mt-3" aria-live="polite" aria-label="پاسخ شخصیت">
            {lastCharacterMessage && <div className="relative mx-auto max-h-44 overflow-y-auto rounded-[2rem] border border-white/20 bg-white/90 px-5 py-4 text-right text-[#21172a] shadow-2xl backdrop-blur-md sm:px-7 sm:py-5">
              <span className="absolute -top-3 right-1/2 h-6 w-6 translate-x-1/2 rotate-45 border-l border-t border-white/20 bg-white/90" aria-hidden="true" />
