...
const chatRateLimiter = rateLimit({ windowMs: 60000, limit: 30, standardHeaders: 'draft-8', legacyHeaders: false, message: { error: 'تعداد درخواست‌ها بیش از حد مجاز است. لطفاً کمی بعد دوباره تلاش کنید.' } });
const staticRateLimiter = rateLimit({ windowMs: 60000, limit: 120, standardHeaders: 'draft-8', legacyHeaders: false });
...
app.post('/api/chat', chatRateLimiter, async (req, res) => {
  const { message, characterId, history } = req.body as { message?: unknown; characterId?: unknown; history?: unknown };
  ...
  try {
    const result = await generateResponse({ message: message.trim(), character, history: safeHistory });
    res.json(result);
  } catch (error) {
...
async function startServer(): Promise<void> {
  ...
  else { const distPath = path.join(process.cwd(), 'dist'); app.use(express.static(distPath)); app.get('*', staticRateLimiter, (_req, res) => res.sendFile(path.join(distPath, 'index.html'))); }
