...
app.get('/api/characters', async (_req, res) => { try { res.json(await listCharacters()); } catch (error) { console.error('Character list error', error); res.status(500).json({ error: 'بارگذاری شخصیت‌ها ناموفق بود.' }); } });
app.get('/api/models', (_req, res) => res.json(providerConfigs));

app.post('/api/chat', async (req, res) => {
  const { message, characterId, provider, model, history, character: clientCharacter } = req.body as { message?: unknown; characterId?: unknown; provider?: unknown; model?: unknown; history?: unknown; character?: Record<string, any> };
  ...
  let character: ServerCharacter | null = await getCharacter(characterId);
  if (!character && clientCharacter?.source === 'custom' && clientCharacter.id === characterId && typeof clientCharacter.systemInstructions === 'string') character = normalizeCharacter(clientCharacter, 'custom');
  ...
  try { const result = await generateResponse({ message: message.trim(), character, provider: provider as Provider, model: typeof model === 'string' ? model : undefined, history: safeHistory }); res.json(result); }
  catch (error) { console.error('Chat provider error', error); res.status(502).json({ error: error instanceof Error ? error.message : 'ارتباط با سرویس هوش مصنوعی ناموفق بود.' }); }
});
